...
とくに2画面ファイラの[あふ(afxw)](http://akt.d.dooo.jp/akt_afxw.html)について説明します．  
...
独習シリーズっぽく，一人で学べる独習あふを目指した文書．
あふの正式名称はあふｗだが，ここではあふと書く．文章の視認性的にひらがなのあふは埋もれて見づらいため，だいたいAFXWと書かく．

## Todo
* 文章が体言止めで終わってたり，違和感があったら要修正
* TBDと書いてある演習等は，より詳細な演習があったほうがいいと思ったやつ．
 * 時間の関係で詳しく書けなかったので，やってみて思いついた演習を追加して
...
6. もし目的のフォルダ位置が探したファイルと違う位置にある場合はもう一つエクスプローラーを起動して移動先まで同上

そのほか，コマンドプロンプトの立ち上げ時にカレントディレクトリが違って毎回cdしないといけないとか，gnuplotのカレントディレクトリが同様に違ったりと，いろいろなわずらわしさがあります．  
（エクスプローラのファイルパス入力欄でcmdと打つとそこをカレントディレクトリにしてコマンドプロンプトが起動するっていう小技を知っていると楽）  
...
この演習中に，迷ったらF1でヘルプ表示，もしくはCTRL+ALT+SHIFT+任意キーの同時押しで該当キーのヘルプ表示の機能があることを覚えておくこと． 
...
afxw64/installフォルダにあるvcredist_x64.exeをインストール．もし，pdfの出力回りで動作しなかったらこれじゃないかも．x86が正しい気もする．
（afxexecに必要）

3. まずはダブルクリックして起動．そして，zキーを押して，そのタブのaboutでオートメーションサーバを登録をクリック．（afxexecに必要）
（登録と解除がトグル）

4. そのタブのプログラム（６）におけるエディタを自分のエディタに変更．
デフォルトは下記のsublimetextになっているが同梱していないため，notepad.exeか自分の使っているエディタにする．
```
...
### 課題
どんなことができるのか．`F1`キーを押して確認してみよ． 
また，各キーを`c-a-s-任意のキー`の同時押しでそのキーのヘルプが出る． 
...
なお，`Z`の設定フォント（４）で，表示のフォントを変えるなどのより詳細な設定ができる． 
...
```
## 表示がおかしい・固まった
* `Z`による設定画面が起動したままになっていないか？これが起動すると本体がかまっている．
* 数字の`1`キーを押してみる．おそらく，分割表示になっている．数字の1,2,3,1と押してみて，もとに戻ったらそれ．（下記，分割表示を参照）．
...
`Enter`キーを押したときに，対象がディレクトリではなくファイルだった場合には，その対象の拡張子に応じて動作が切り替わる． 
主に以下のファイルで動作が切り替わる．
* 画像ファイル
* 圧縮ファイル
* オフィスファイル
...
基本，`Enter`がデフォルト動作，`s-Enter`が少しマイナーな動作である． 
また，`c-Enter`はエクスプローラーでファイルをダブルクリックしたときと同じ動作であり，「関連付け実行」と呼ぶ． 
つまり，exeを実行したり，フォルダをエクスプローラーで開いたり，ワードやエクセルファイルを開いたりする動作であり，起動するとあふから離れることになる実行である． 

以下ではそれぞれのファイルについての挙動を詳しく述べる． 

なお，以下のサイトなど，Webのコンバータサービスを使ってファイルが開けるかどうかのサンプルファイルを作るのもよい． 

* [anyconv](https://anyconv.com/ja/)

## テキストファイルの閲覧
特定の拡張子ではない場合のすべてでテキストビューアとして開かれる． 
...
ショートカットとして，vimのように`H, J, K, L`で上下左右のカーソルが動く． 
`G`，`s-G`で文頭，文末，`V`で終了など（あとで述べるが`VV`の２キーストロークで開いて閉じることができる）． 
そのほか，`c-Space`でカーソルが出る．その際，`H, J, K, L`での移動の動作も変わる． 

また，テキストビュー中で`Tab`キーを押すとバイナリファイルとして閲覧することもできる． 
このようにバイナリ・テキストを切り替えられるとたまに便利である． 

また，`V`キー(View)で，すべてのファイルをテキストだと思って開くこともできる． 
...
テキストを編集する場合には，`E`キー(Edit)でテキスト編集ソフトでも開くことができる． 
...
9. 閲覧中にTabを打って16進数表示と通常の表示を切り替えよ．もう一度Tabでトグルする．
...
`c-Enter`の関連付けで開いてもよいが，拡張子が.txtでないとうまく開かないことが多い． 
...
対応する拡張子は以下であり，通常ほぼすべてのファイルを開くことができる． 
```
...
圧縮ファイルの深い階層に入っているときからもとに戻るときは，いちいち矢印を押して戻らずとも以下のショートカットで簡単に抜けられる． 
...
```
jpg png gif bmp ppm pgm pbm jpeg jpe tif tiff webp jp2 ico svg wmf emf eps
```
モードには通常のファイルマネージャーモードと，テキストモードとこの画像モードの三つがある． 
...
なお，少し拡張したら，HEIFも閲覧可能にできる．必要であれば，[afxw_extention.md](afxw_extention.md)を参照のこと．
...
4. オフィスファイルを`s-Enter`で開き，内部の画像を取り出せ（要画像の入ったワードやパワーポイントファイル）
...
**オフィスのテキスト閲覧の拡張の方法**
1. afxtoolsのafxexecを使って，コマンドラインの標準出力をあふのメッセージ窓で受け取れるようにする（要オートメーションサーバ）
...
オフィスファイルと同様に，`Enter`キーで先頭7だけテキスト表示することができ，全文は同様に`,`で参照可能である． 
...
設定の詳細は[afxw_extention.md](afxw_extention.md)のpdf表示関連の設定を参照のこと． 
...
もし多様するようであれば，設定を変える必要があるかもしれない． 
...
ファイルをマーク，千選択するときに，ひとつづつ選択するのはコストが高い．
...
5. 【応用】 7zipのコマンドライン引数を調べ，`a-P`の圧縮メニューに選択肢を追記せよ．
...
ここでは，用語を展開に統一した． 
英語ならdecodeとかdecompressでいいので悩む必要がない． 
...
* [設定](afxw_extention.md)
* [キーバインド](afxw_keybind.md)
* [仮想キーコード：afxw.key編集用](virtual_key_codes.md)
* [内部命令一覧](internal_operation.md)
拡張時の指針を決めたときの考え方．これも読む必要はさらにないかも．[リンク](afxw_column.md)
